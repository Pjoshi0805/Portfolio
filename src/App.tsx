    ...
    >
      <div className="max-w-7xl mx-auto">
        <motion.h2 
          ...
          className="text-[8vw] leading-none mb-32 font-serif text-brand-dark"
        >
          Skills
        </motion.h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-16 gap-y-8">
          {skills.map((skill, index) => (
            <motion.div
              ...
              className="group relative py-8 border-b border-brand-dark/10 flex items-baseline gap-6 cursor-none"
            >
              <span className="text-[10px] tracking-widest font-bold opacity-30">
                {index < 9 ? `0${index + 1}` : index + 1}
              </span>
              <h3 className="text-3xl lg:text-5xl font-serif text-brand-dark transition-all duration-500 group-hover:italic group-hover:translate-x-4">
                {skill.name}
              </h3>
            </motion.div>
          ))}
